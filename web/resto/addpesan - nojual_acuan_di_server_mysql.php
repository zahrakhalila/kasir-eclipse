...
if ($nojual!='' ) {
$query = "select nojual as sno from jual_m where nojual='$nojual'";
$result = mysqli_query($link, $query) or die('Errorquery:  '.$query);
$ketemuno=mysqli_num_rows($result);
if ($ketemuno > 0){
	$query = "UPDATE jual_m set nomormeja='$_POST[nomormeja]',kodeplg='$_POST[kodeplg]',tgljual='$_POST[tgljual]',nilai='$_POST[nilai]' where noimei='$_POST[noimei]' and nojual='$nojual'";
	$result = mysqli_query($link, $query) or die('Errorquery:  '.$query);
} else {
	$stgl=$_POST['tgljual'];
